...
import { useSquadData } from '@/hooks/useSquadData'; // Using existing hook
import { Search, Plus, User, Edit, Loader2, X } from 'lucide-react';
import { useToast } from '@/hooks/use-toast';
import { Checkbox } from '@/components/ui/checkbox';
import { Label } from '@/components/ui/label';
import { useTheme } from '@/hooks/useTheme';
import { ScrollArea } from '@/components/ui/scroll-area'; // For scrollable lists
...
interface PlayerSearchModalProps {
  ...
  position?: string; // Optional: Prefill position in create form
}

const PlayerSearchModal = ({ isOpen, onClose, onPlayerSelect, position }: PlayerSearchModalProps) => {
  // Use squad data hook also for card types and saving
  ...
  // Memoize initial state to avoid re-renders
  const initialNewPlayerState = useMemo(() => ({
    ...
    price: 0,
    is_evolution: false,
    // GK stats only needed if position is GK
    diving: position === 'GK' ? 70 : undefined,
    handling: position === 'GK' ? 70 : undefined,
    kicking: position === 'GK' ? 70 : undefined,
    reflexes: position === 'GK' ? 70 : undefined,
    speed: position === 'GK' ? 70 : undefined, // GK Speed
    positioning: position === 'GK' ? 70 : undefined, // GK Positioning
    is_goalkeeper: position === 'GK',
  }), [position, cardTypes]); // Depend on position and cardTypes

  const [newPlayer, setNewPlayer] = useState<Partial<PlayerCard>>(initialNewPlayerState);

  // Debounced search could be an improvement here if performance is an issue
  useEffect(() => {
    if (searchTerm.trim()) {
  ...
  useEffect(() => {
    if (isOpen) {
       // Recalculate initial state based on current props
       const currentInitialState = {
         ...initialNewPlayerState,
         position: position || 'CM',
         card_type: cardTypes.find(ct => ct.is_default)?.id || cardTypes[0]?.id || '',
         is_goalkeeper: position === 'GK',
         // Reset GK stats based on position
         diving: position === 'GK' ? 70 : undefined,
         handling: position === 'GK' ? 70 : undefined,
         kicking: position === 'GK' ? 70 : undefined,
         reflexes: position === 'GK' ? 70 : undefined,
         speed: position === 'GK' ? 70 : undefined,
         positioning: position === 'GK' ? 70 : undefined,
       };
       setNewPlayer(currentInitialState);
       setShowCreateForm(false); // Default to search view
       setEditingPlayer(null);
       setSearchTerm(''); // Clear search on open
    }
  }, [isOpen, position, cardTypes, initialNewPlayerState]); // Add initialNewPlayerState dependency

  const handleCreateOrUpdate = async () => {
    if (!newPlayer.name?.trim()) {
      toast({ title: "Name Required", description: "Player name cannot be empty.", variant: "destructive" });
      return;
    }
    setIsSaving(true);
    const playerToSave: Partial<PlayerCard> = {
      ...newPlayer,
      id: editingPlayer ? editingPlayer.id : undefined, // Include ID if updating
      is_goalkeeper: newPlayer.position === 'GK', // Ensure is_goalkeeper is set correctly
    };

    // Remove GK stats if not a GK, remove outfield stats if GK
    if (playerToSave.position !== 'GK') {
         delete playerToSave.diving; delete playerToSave.handling; delete playerToSave.kicking;
         delete playerToSave.reflexes; delete playerToSave.speed; delete playerToSave.positioning;
    } else {
        delete playerToSave.pace; delete playerToSave.shooting; delete playerToSave.passing;
        delete playerToSave.dribbling; delete playerToSave.defending; delete playerToSave.physical;
    }


    const savedPlayer = await savePlayer(playerToSave); // savePlayer should handle insert/update based on ID
    setIsSaving(false);

    if (savedPlayer) {
      onPlayerSelect(savedPlayer); // Pass saved player back
      onClose(); // Close modal on success
      toast({ title: editingPlayer ? "Player Updated" : "Player Created", description: `${savedPlayer.name} saved.` });
    }
    // Error toast handled within useSquadData hook
  };

  const handleEditPlayer = (player: PlayerCard) => {
    setEditingPlayer(player);
    // Ensure all relevant fields are copied, especially GK stats if applicable
  ...
  const resetAndClose = () => {
      onClose();
      // No need to reset form state here, useEffect on isOpen handles it
  }
  ...
  const getCardTypeStyle = (cardTypeId: string | undefined) => {
    ...
    return { background: currentTheme.colors.muted, color: currentTheme.colors.mutedForeground }; // Muted fallback
  };
  ...
  const handleInputChange = useCallback((field: keyof PlayerCard, value: string | number | boolean) => {
      ...
      if (typeof initialNewPlayerState[field] === 'number') {
          processedValue = value === '' ? '' : (Number(value) || 0); // Allow empty string temporarily, default to 0 if invalid
      }
      setNewPlayer(prev => ({ ...prev, [field]: processedValue }));
  }, []);

   // Handle blur for number inputs to finalize value
  const handleNumberBlur = useCallback((field: keyof PlayerCard) => {
       ...
       // Apply constraints (e.g., rating 0-99)
       if(field === 'rating' || ['pace', 'shooting', 'passing', 'dribbling', 'defending', 'physical', 'diving', 'handling', 'kicking', 'reflexes', 'speed', 'positioning'].includes(field)) {
            finalValue = Math.max(0, Math.min(99, finalValue));
       } else {
            finalValue = Math.max(0, finalValue); // General non-negative
       }
  ...
  const gkStats: (keyof PlayerCard)[] = ['diving', 'handling', 'kicking', 'reflexes', 'speed', 'positioning']; // Assuming speed/pos are GK specific


  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && resetAndClose()}>
      <DialogContent className="max-w-xl max-h-[90vh] flex flex-col glass-card border-border/20 text-white">
        <DialogHeader>
        ...
        {!showCreateForm && (
          <div className="flex flex-col gap-4 pt-4 flex-grow min-h-0">
            ...
            <ScrollArea className="flex-grow pr-3 -mr-3 max-h-[40vh]"> {/* Scrollable results */}
                    ...
                    {suggestions.map((player) => (
                        <div key={player.id} className="flex items-center justify-between p-2 rounded-lg group hover:bg-white/10 transition-colors cursor-pointer" onClick={() => onPlayerSelect(player)}>
                            <div className="flex items-center gap-3 flex-1 overflow-hidden">
                                <Badge style={getCardTypeStyle(player.card_type)} className="text-xs font-bold px-1.5 py-0.5">{player.rating}</Badge>
        ...
        {/* --- CREATE/EDIT VIEW --- */}
        {showCreateForm && (
          <ScrollArea className="flex-grow min-h-0 pr-3 -mr-3 mt-4"> {/* Scrollable form */}
            <div className="space-y-4 pb-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="space-y-1"><Label htmlFor="cardType">Card Type</Label>
                        {squadDataLoading ? <Skeleton className="h-9 w-full" /> :
                ...
                {/* Is Evolution */}
                <div className="flex items-center space-x-2 pt-2"><Checkbox id="isEvolution" checked={newPlayer.is_evolution} onCheckedChange={(checked) => handleInputChange('is_evolution', !!checked)} /><Label htmlFor="isEvolution" className="text-sm font-medium">Is Evolution Card?</Label></div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-x-4 gap-y-3">
                        {(isGoalkeeper ? gkStats : outfieldStats).map(stat => (
                            <div key={stat} className="space-y-1">
                                <Label htmlFor={stat} className='capitalize text-sm'>{stat}</Label>
           ...
           <DialogFooter className="mt-auto pt-4 border-t border-border/20">
                <Button onClick={() => { setShowCreateForm(false); setEditingPlayer(null); setNewPlayer(initialNewPlayerState); }} variant="outline" disabled={isSaving}>Back to Search</Button>
