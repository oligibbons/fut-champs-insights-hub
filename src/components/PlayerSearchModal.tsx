...
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogFooter } from '@/components/ui/dialog'; // Ensure DialogFooter is imported if used explicitly
...
const PlayerSearchModal = ({ isOpen, onClose, onPlayerSelect, position }: PlayerSearchModalProps) => {
  ...
  const getCardTypeStyle = (cardTypeId: string | undefined) => { /* ... (logic remains the same) ... */
  ...
  const handleInputChange = useCallback((field: keyof PlayerCard, value: string | number | boolean) => { /* ... (logic remains the same) ... */
  ...
  const handleNumberBlur = useCallback((field: keyof PlayerCard) => { /* ... (logic remains the same) ... */
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && resetAndClose()}>
      <DialogContent className="max-w-xl max-h-[90vh] flex flex-col glass-card border-border/20 text-white">
        ...
        {/* --- SEARCH VIEW --- */}
        {!showCreateForm ? (
          // --- FIX: Ensure DialogFooter is INSIDE this div ---
          <div className="flex flex-col gap-4 pt-4 flex-grow min-h-0">
            <div className="relative shrink-0"> {/* Prevent input from shrinking */}
            ...
            <ScrollArea className="flex-grow pr-3 -mr-3 min-h-[100px]"> {/* Min height + scroll */}
             ...
             {/* Footer now correctly inside the main div */}
             <DialogFooter className="mt-auto pt-4 border-t border-border/20 shrink-0">
        ...
        {showCreateForm && (
            ...
            <div className="flex flex-col flex-grow min-h-0 pt-4">
                 ...
                 </ScrollArea> {/* End Scrollable form content */}
                 <DialogFooter className="mt-auto pt-4 border-t border-border/20 shrink-0"> {/* Footer pushed to bottom */}
                    <Button onClick={() => { setShowCreateForm(false); setEditingPlayer(null); /* No need to reset newPlayer here */ }} variant="outline" disabled={isSaving}>Back to Search</Button>
            ...
            </div> // End flex container for create view
        )}
