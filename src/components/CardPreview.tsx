...
// Note: Playstyle is imported here but the usage below is wrapped in a safe static container.
import { Playstyle } from './Playstyle'; 

// --- Resolved Theme Colors (Using explicit HEX for Image Generation Reliability) ---
// This ensures colors are rendered correctly by the canvas engine.
const RESOLVED_COLORS = {
    PRIMARY: '#75baff',       // Light Blue/Cyan (Primary)
    WIN: '#86efac',           // FIFA Green (Success)
    LOSS: '#f87171',          // FIFA Red (Failure)
    BG_CARD: '#1b2133',       // Deep Charcoal Blue (Card Background)
    BG_MAIN: '#111111',       // Near Black (Main Background)
};
...
const calculateAllStats = (runData?: WeeklyPerformance | null, allRunsData?: WeeklyPerformance[] | null, cardType?: 'run' | 'overall'): CalculatedStats => {
    // --- NOTE: This logic needs to be fully implemented in the final product. ---
    if (cardType === 'run' && runData) {
        const games = runData.games?.filter(g => g) ?? [];
        const stats: CalculatedStats = {
            title: runData.custom_name || `WEEK ${runData.week_number}`, gamesPlayed: games.length,
            ...
            xgDifferential: 0.5, cpsScore: calculateCPS(games),
            highestScorer: { name: "Mbappé", goals: 12 }, highestAssister: { name: "Zidane", assists: 8 }, passAccuracy: 85,
        };
...
const ThemedStatBlock: React.FC<CustomStatBlockProps> = ({ options, optionId, Icon, label, value, unit }) => {
    ...
    return (
        <div className={cn(
             "relative flex flex-col p-0 overflow-hidden rounded-xl border border-white/20", // Border to mimic deep inset
             "bg-white/5" // Keep light translucent background
        )}>
            {/* Header Box (Themed Color, Heavy Weight) */}
            <div className="flex items-center justify-between px-3 py-1.5 font-bold text-[10px] uppercase tracking-widest leading-tight"
            ...
            {/* Content Area (Visually large number) */}
...
const CardPreview: React.FC<CardPreviewProps> = ({ runData, allRunsData, options, cardType, userScreenName, activeStatCount, totalRows }) => {
  ...
  // --- Background Style (Dark Charcoal Gradient) ---
    ...
    <div 
      ...
      <div className="h-full w-full absolute inset-0" style={gridTemplate as React.CSSProperties}>
        ...
        <div className="flex flex-col justify-between p-3 rounded-xl bg-white/5" style={{ gridArea: '1 / 1 / 2 / 2' }}>
            {/* FIX: Static SVG Placeholder for Logo (Guaranteed to render) */}
            <svg xmlns="http://www.w3.org/2000/svg" className="h-9 w-9 text-primary flex-shrink-0" viewBox="0 0 24 24" fill="none" stroke={RESOLVED_COLORS.PRIMARY} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                <path d="M4 14.9L10 20.9 20 4.9"/>
            </svg>
            <div className='pt-2'>
        ...
        <div className="p-3 rounded-xl bg-white/5 flex items-center justify-center" style={{ gridArea: '1 / 2 / 2 / 4' }}>
            {/* FIX: Static Text Placeholder for Playstyle (Removed Playstyle component to prevent serialization failure) */}
            <div className="h-full w-full flex flex-col items-center justify-center text-center text-white/60 text-sm font-semibold">
                <span>[PLAYSTYLE DATA]</span>
