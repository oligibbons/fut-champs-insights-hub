...
// --- Interfaces and Default Settings from your original file ---
...
interface DataSyncContextType {
  ...
  updateWeek: (weekId: string, updates: Partial<WeeklyPerformance>) => Promise<void>;
  updateGame: (weekId: string, gameId: string, updates: any) => Promise<void>;
  getCurrentWeek: () => WeeklyPerformance | undefined;
  ...
  setWeeklyData: () => void;
  calculatePlayerStats: () => PlayerPerformance[];
  settings: Settings;
...
export const DataSyncProvider = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => {
    ...
    if (currentWeek && currentWeek.games.length >= 15 && !currentWeek.isCompleted) {
      supabaseData.updateWeek(currentWeek.id, { isCompleted: true, endDate: new Date().toISOString() });
    }
