...
import { PlusCircle, Loader2, BarChart2, Star, ListChecks, FlagOff, ArrowLeft } from 'lucide-react';
...
const CurrentWeek = () => {
    ...
    const handleSaveGame = async () => {
        ...
        if (selectedRun) {
            ...
            if (games.length + 1 >= 15 && !selectedRun.is_completed) {
            ...
            <div className="flex justify-between items-center">
                ...
                {!selectedRun.is_completed && (
                    <div className="flex gap-2">
                        <Button onClick={() => setGameToEdit({} as Game)} disabled={games.length >= 15}><PlusCircle className="mr-2 h-4 w-4" /> Record Game</Button>
                        <Button variant="outline" onClick={handleEndRun}><FlagOff className="mr-2 h-4 w-4" /> End Run</Button>
                    </div>
                )}
            </div>

            <Dialog open={!!gameToEdit} onOpenChange={() => setGameToEdit(null)}>
                <DialogContent className="max-w-4xl h-[90vh] flex flex-col">
                    <DialogHeader>
                        <DialogTitle>{gameToEdit?.id ? 'Edit Game' : 'Record a Game'}</DialogTitle>
                        <DialogDescription>
                            {gameToEdit?.id ? `Update details for game #${gameToEdit.game_number}` : `Enter details for game #${nextGameNumber}`}
                        </DialogDescription>
                    </DialogHeader>
                    <div className="flex-1 min-h-0">
                        <GameRecordForm existingGame={gameToEdit} squads={squads} weekId={selectedRun.id} nextGameNumber={nextGameNumber} onSave={handleSaveGame} onCancel={() => setGameToEdit(null)} />
                    </div>
                </DialogContent>
            </Dialog>
            ...
            <Tabs defaultValue="overview" className="w-full">
              ...
              <TabsContent value="overview" className="mt-4">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                    {games.map(game => (<GameCard key={game.id} game={game} onEdit={setGameToEdit} onDelete={(id) => setItemToDelete({type: 'game', id})} />))}
                </div>
              </TabsContent>
               <TabsContent value="gamelist" className="mt-4 space-y-2">
                    {games.map(game => (<GameListItem key={game.id} game={game} onEdit={setGameToEdit} onDelete={(id) => setItemToDelete({type: 'game', id})} />))}
               </TabsContent>
