    ...
    <>
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="relative"><Search className="absolute left-3 top-1/2 transform -translate-y-1/2 h-4 w-4 text-muted-foreground" /><Input placeholder="Search users..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-10" /></div>
          <div className="rounded-lg border overflow-hidden"><div className="overflow-x-auto"><table className="w-full"><thead><tr className="border-b"><th className="px-4 py-3 text-left text-muted-foreground">User</th><th className="px-4 py-3 text-left text-muted-foreground">Created</th><th className="px-4 py-3 text-left text-muted-foreground">Status</th><th className="px-4 py-3 text-left text-muted-foreground">Actions</th></tr></thead><tbody>
            {filteredUsers.map((user) => (<tr key={user.id} className="border-b"><td className="px-4 py-3"><p className="font-medium text-foreground">{user.display_name || user.username}</p><p className="text-sm text-muted-foreground">@{user.username}</p></td><td className="px-4 py-3 text-foreground">{new Date(user.created_at).toLocaleDateString()}</td><td className="px-4 py-3"><div className="flex items-center gap-2">{user.is_admin && <Badge>Admin</Badge>}{user.is_banned ? <Badge variant="destructive">Banned</Badge> : <Badge variant="secondary">Active</Badge>}</div></td><td className="px-4 py-3"><div className="flex items-center gap-2"><Button size="sm" variant="outline" onClick={() => setEditingUser(user)}><Edit className="h-4 w-4" /></Button><Button size="sm" variant="destructive" onClick={() => setDeleteConfirmation(user)}><Trash2 className="h-4 w-4" /></Button></div></td></tr>))}
          </tbody></table></div></div>
        </CardContent>
      </Card>
      <Dialog open={!!editingUser} onOpenChange={() => setEditingUser(null)}><DialogContent><DialogHeader><DialogTitle>Edit User</DialogTitle></DialogHeader>{editingUser && (<div className="space-y-4 py-4"><div><Label>Username</Label><Input value={editingUser.username || ''} onChange={(e) => setEditingUser({ ...editingUser, username: e.target.value })} /></div><div><Label>Display Name</Label><Input value={editingUser.display_name || ''} onChange={(e) => setEditingUser({ ...editingUser, display_name: e.target.value })} /></div><div className="flex items-center justify-between"><div className="space-y-1"><Label>Admin Status</Label><p className="text-sm text-muted-foreground">Grant admin privileges</p></div><Switch checked={editingUser.is_admin} onCheckedChange={(checked) => setEditingUser({ ...editingUser, is_admin: checked })} /></div><div className="flex items-center justify-between"><div className="space-y-1"><Label>Account Status</Label><p className="text-sm text-muted-foreground">{editingUser.is_banned ? 'Banned' : 'Active'}</p></div><Button variant="outline" onClick={() => setEditingUser({ ...editingUser, is_banned: !editingUser.is_banned })} className={editingUser.is_banned ? 'text-green-600 border-green-600' : 'text-red-600 border-red-600'}>{editingUser.is_banned ? (<><Unlock className="h-4 w-4 mr-2" />Unban</>) : (<><Lock className="h-4 w-4 mr-2" />Ban</>)}</Button></div><div className="flex gap-3 justify-end pt-4"><Button variant="outline" onClick={() => setEditingUser(null)}>Cancel</Button><Button onClick={handleSaveUser}><Save className="h-4 w-4 mr-2" />Save</Button></div></div>)}</DialogContent></Dialog>
      <AlertDialog open={!!deleteConfirmation} onOpenChange={() => setDeleteConfirmation(null)}><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>Delete User?</AlertDialogTitle><AlertDialogDescription>This will delete the user's profile data. This action cannot be undone.</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel>Cancel</AlertDialogCancel><AlertDialogAction onClick={handleDeleteUser} className="bg-red-600 hover:bg-red-700">Delete</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog>
    </>
  );
};
...
const StatCard = ({ icon: Icon, title, value, isStatus }: { icon: React.ElementType, title: string, value: string | number, isStatus?: boolean }) => (
    <div className="p-4 bg-card-foreground/5 rounded-xl">
        <div className="flex items-center gap-3 mb-2"><Icon className={`h-5 w-5 ${isStatus && value === 'online' ? 'text-green-500' : 'text-primary'}`} /><span className="font-medium text-foreground">{title}</span></div>
        ...
        <Card>
            ...
            <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-4">
                    ...
                    <div className="space-y-2"><Label htmlFor="card-name">Card Name</Label><Input id="card-name" placeholder="e.g., Team of the Season" value={newCardType.name} onChange={(e) => setNewCardType({ ...newCardType, name: e.target.value })} /></div>
                    ...
                    <div className="pt-4"><h4 className="font-semibold text-sm mb-2">Live Preview:</h4><CardPreview name={newCardType.name} primaryColor={newCardType.primary_color} secondaryColor={newCardType.secondary_color} highlightColor={newCardType.highlight_color} /></div>
                    <Button onClick={handleSave}><Plus className="h-4 w-4 mr-2" />Save New Card Type</Button>
                </div>
                <div className="space-y-4"><h3 className="font-semibold">Existing Card Types ({gameVersion})</h3><div className="space-y-2 max-h-96 overflow-y-auto pr-2">{cardTypes.map(card => (<div key={card.id} className="flex items-center justify-between p-2 bg-secondary/50 rounded-lg"><div className="flex items-center gap-3"><div className="w-5 h-5 rounded-full border border-border" style={{background: `linear-gradient(135deg, ${card.primary_color} 50%, ${card.secondary_color} 50%)`}}></div><span className="font-medium">{card.name}</span></div><div className="flex items-center gap-2"><Dialog><DialogTrigger asChild><Button variant="outline" size="icon" className="h-7 w-7" onClick={() => setEditingCardType(card)}><Edit className="h-4 w-4" /></Button></DialogTrigger><DialogContent><DialogHeader><DialogTitle>Edit Card Type</DialogTitle></DialogHeader>{editingCardType && editingCardType.id === card.id && (<div className="space-y-4 pt-4"><div className="space-y-2"><Label htmlFor="edit-card-name">Card Name</Label><Input id="edit-card-name" value={editingCardType.name} onChange={(e) => setEditingCardType({ ...editingCardType, name: e.target.value })} /></div><div className="grid grid-cols-3 gap-4"><div className="space-y-2"><Label htmlFor="edit-primary-color">Primary</Label><Input id="edit-primary-color" type="color" value={editingCardType.primary_color} onChange={(e) => setEditingCardType({ ...editingCardType, primary_color: e.target.value })} /></div><div className="space-y-2"><Label htmlFor="edit-secondary-color">Secondary</Label><Input id="edit-secondary-color" type="color" value={editingCardType.secondary_color} onChange={(e) => setEditingCardType({ ...editingCardType, secondary_color: e.target.value })} /></div><div className="space-y-2"><Label htmlFor="edit-highlight-color">Highlights</Label><Input id="edit-highlight-color" type="color" value={editingCardType.highlight_color} onChange={(e) => setEditingCardType({ ...editingCardType, highlight_color: e.target.value })} /></div></div><div className="pt-4"><h4 className="font-semibold text-sm mb-2">Live Preview:</h4><CardPreview name={editingCardType.name} primaryColor={editingCardType.primary_color} secondaryColor={editingCardType.secondary_color} highlightColor={editingCardType.highlight_color} /></div><Button onClick={handleUpdate}>Save Changes</Button></div>)}</DialogContent></Dialog><Button variant="destructive" size="icon" className="h-7 w-7" onClick={() => handleDelete(card.id)}><Trash2 className="h-4 w-4" /></Button></div></div>))}</div></div>
            </CardContent>
        </Card>
    );
};
...
const AchievementCreator = () => {
    ...
    return (
      <Card>
        <CardHeader className="flex-row items-center justify-between"><CardTitle>Achievement Management</CardTitle><Button onClick={startNew}><Plus className="h-4 w-4 mr-2" /> Create New</Button></CardHeader>
        <CardContent className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="md:col-span-1 space-y-4 p-4 rounded-lg bg-card-foreground/5">{editingAchievement ? (<><h3 className="font-semibold">{editingAchievement.id.startsWith('new-') ? 'Create' : 'Edit'} Achievement</h3><Input placeholder="Achievement Name" value={editingAchievement.name} onChange={(e) => setEditingAchievement({...editingAchievement, name: e.target.value })} /><Textarea placeholder="Description" value={editingAchievement.description} onChange={(e) => setEditingAchievement({...editingAchievement, description: e.target.value })} /><Select value={editingAchievement.tier} onValueChange={(value: Achievement['tier']) => setEditingAchievement({...editingAchievement, tier: value })}><SelectTrigger><SelectValue/></SelectTrigger><SelectContent><SelectItem value="bronze">Bronze</SelectItem><SelectItem value="silver">Silver</SelectItem><SelectItem value="gold">Gold</SelectItem><SelectItem value="platinum">Platinum</SelectItem></SelectContent></Select><div><Label>Criteria (JSON)</Label><Textarea placeholder='{ "stat": "wins", "value": 10 }' value={JSON.stringify(editingAchievement.criteria, null, 2)} onChange={(e) => setEditingAchievement({...editingAchievement, criteria: e.target.value as any })} className="font-mono h-32"/></div><div className="flex gap-2"><Button onClick={() => setEditingAchievement(null)} variant="outline" className="w-full"><X className="h-4 w-4 mr-2"/>Cancel</Button><Button onClick={handleSave} className="w-full"><Save className="h-4 w-4 mr-2"/>Save</Button></div></>) : <p className="text-muted-foreground text-center py-10">Select or create an achievement.</p> }</div>
          <div className="md:col-span-2 space-y-2">{achievements.map(ach => (<div key={ach.id} className="flex items-center justify-between p-2 rounded-md hover:bg-card-foreground/10"><div className="flex items-center gap-4"><Badge variant={ach.tier === 'gold' || ach.tier === 'platinum' ? 'default' : 'secondary'}>{ach.tier}</Badge><div><p className="font-medium">{ach.name}</p><p className="text-sm text-muted-foreground">{ach.description}</p></div></div><div className="flex items-center gap-2"><Button variant="ghost" size="icon" onClick={() => setEditingAchievement(ach)}><Edit className="h-4 w-4"/></Button><Button variant="ghost" size="icon" onClick={() => handleDelete(ach.id)}><Trash2 className="h-4 w-4 text-red-500"/></Button></div></div>))}</div>
        </CardContent>
      </Card>
    );
};
    ...
    <div className="min-h-screen">
        ...
        <main className="lg:ml-64 p-4 lg:p-6">
            <div className="space-y-8">
                <div><h1 className="text-3xl font-bold tracking-tight flex items-center gap-3"><ShieldCheck className="h-8 w-8 text-primary" />Admin Panel</h1><p className="text-muted-foreground">Site-wide management and analytics.</p></div>
                <Tabs defaultValue="users" className="w-full">
                    <div className="overflow-x-auto pb-2"><TabsList className="inline-flex w-max">
